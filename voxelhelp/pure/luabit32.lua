local luabit32 = { internal = { } }

local math = math

function luabit32.internal.setmath(newmath)
	math = newmath
end
...
function luabit32.bnot(x)
	x = x % P
	return P - 1 - x
end

function luabit32.band(x, y)
...
function luabit32.lshift(x, s_amount)
...
function luabit32.rshift(x, s_amount)
	if math.abs(s_amount) >= N then return 0 end
	x = x % P
	if s_amount > 0 then
		return math.floor(x * (2 ^ - s_amount))
...
function luabit32.arshift(x, s_amount)
	if math.abs(s_amount) >= N then return 0 end
	x = x % P
	if s_amount > 0 then
		local add = 0
