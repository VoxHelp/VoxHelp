...
function vox_math:generateRandomSeed(iter) -- 
    local time = (world.get_day_time())/iter*vox_math:randInt()
    local  wseed = world.get_seed();
    return time * (wseed * 2)
end


function vox_math:randInt() -- Большие числа (Не зависит от итераций)
    return vox_math:round(vox_math:random()*vox_math:pi()*78319*world:get_day_time(),0)
end

function vox_math:randint(a, b, iteration) -- Числа на отрезке (Зависит от итераций)
    local m = 2385482343
    local aa = 48271      
    local c = 0          
    ...
    return vox_math:abs(vox_math:floor((seed / (m / (b - a + 1))) + a))
end


function vox_math:random()
    return (vox_math:abs(vox_math:sin(3/(world:get_day_time()-0.5))/vox_math:cos(2/(world:get_day_time()-0.5))))
end
